...
def tap_volume_button(serial, adb_path="adb"):
    print(f"[{serial}] 📥 Dumping UI để tìm nút Volume...")
    ...
    for node in root.iter():
        if (
            ...
            and node.attrib.get("content-desc") == "Volume"
            and node.attrib.get("enabled") == "true"
        ):
            bounds = node.attrib.get("bounds", "")
            if bounds:
                x, y = get_center_of_bounds(bounds)
                print(f"[{serial}] ✅ Tap nút Volume tại ({x}, {y})")
                adb_tap(x, y, serial, adb_path)
                return

    print(f"[{serial}] ⚠️ Không tìm thấy nút chỉnh âm lượng.")
