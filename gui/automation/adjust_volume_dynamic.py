...
def wait_until_ui_has(serial, keyword, timeout=10, use_class=False):
    ...
    while time.time() - start < timeout:
        ...
        try:
            ...
            for node in root.iter():
                if use_class and node.attrib.get("class") == keyword:
                    print(f"[{serial}] ✅ Tìm thấy UI class: '{keyword}'")
                    return True
                if (
...
def run(serial, song_name=None, api_key=None, voice_percent=100, music_percent=50):
    ...
    if wait_until_ui_has(serial, "reel_camera_gallery_button_delegate", timeout=10):
        ...
        if not wait_until_ui_has(serial, "android.widget.SeekBar", timeout=20, use_class=True):
            print(f"[{serial}] ❌ Không tìm thấy thanh chỉnh volume.")
        else:
            adjust_volume_dynamic.adjust_volume_dynamic(serial, voice_percent, music_percent)
